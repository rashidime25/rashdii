...
| 👤 **ورود بدون ثبت‌نام** | اولین ورود با `TiTaN` / `TiTaN` (قابل تغییر با `TITAN_ADMIN_USER` و `TITAN_ADMIN_PASS`) — بعد از ورود از تنظیمات رمز را عوض کنید |
...
4. به آدرس سرویس + `/login` بروید و با `TiTaN` / `TiTaN` وارد شوید (رمز پیش‌فرض؛ فوراً عوضش کنید).

### 🌐 Render
1. در [render.com](https://render.com) → **New → Web Service** → ریپازیتوری را وصل کنید.
2. `render.yaml` با runtime داکر شناسایی می‌شود.
3. بعد از دیپلوی به `/login` بروید و با `TiTaN` / `TiTaN` وارد شوید.
...
# → http://localhost:8000/login  (کاربر: TiTaN — رمز: TiTaN)
```
> بدون Xray، پنل در حالت mock کار می‌کند (همه امکانات مدیریتی فعال است؛ فقط ترافیک واقعی عبور نمی‌کند).

## ورود اولیه

1. `<your-domain>/login` → ورود با `TiTaN` / `TiTaN`
2. ⚠️ از **تنظیمات → امنیت** رمز را عوض کنید («رمز عبور فعلی» = `TiTaN`)
...
| `/api/users/<uid>/client-config` | GET | کانفیگ JSON آماده‌ی ایمپورت (همان تنظیمات سرور) |
| `/api/users/<uid>/link-test` | POST | تست واقعی اتصال کانفیگ از روی سرور (تونل واقعی + زمان پاسخ) |
| `/api/recipes` | GET | پروفایل‌های آماده + واژگان UI (flow، پلن‌ها، حالت‌های XHTTP) |
...
## کانفیگ‌ساز (پروفایل‌های آماده + تنظیم موتور)

ساخت کانفیگ دو مرحله دارد: انتخاب یک **پروفایل آماده**، و در صورت نیاز، دست‌کاری **تنظیم موتور**.

| پروفایل | چه زمانی |
|---|---|
| 🛡 مقاوم (Reality + Vision) | اپراتورهای ایران؛ بدون گواهی و دامنه، مقاوم‌ترین حالت امروز |
| 🌐 CDN (XHTTP + TLS) | IP سرور تمیز نیست یا پشت Cloudflare هستی |
| 🔀 سازگاری حداکثری (WS + TLS) | کم‌ترین ریسک ناسازگاری با همه‌ی کلاینت‌ها |
| 🎮 گیمینگ | بافر کوچک و پلن policy با تأخیر کم |
| ✈️ تلگرام/متن‌محور | برای مصرف کم و پایداری روی شبکه‌ی ضعیف |

نکته‌ی مهم: پروفایل فقط فیلدهای خالی را پر می‌کند؛ هر مقداری که خودت در فرم ست کرده باشی
دست‌نخورده می‌ماند. بخشی از پروفایل به **تنظیمات پنل** مربوط است (مثل Fragment)، و پنل
همان‌ها را اعمال و در پاسخ API گزارش می‌کند (`recipe_settings`).

**تنظیم موتور** (بخش «کارگاه کانفیگ» در تب کانفیگ‌ها):

| کلید | اثر |
|---|---|
| `sock_tfo` / `sock_nodelay` / `sock_keepalive` / `sock_user_timeout` | سوکت TCP: یک رفت‌وبرگشت کمتر، حذف تأخیر Nagle، زنده‌نگه‌داشتن اتصال روی NAT اپراتور |
| `sock_congestion` | الگوریتم ازدحام (`bbr`، `cubic`) — روی هسته‌های اشتراکی معمولاً بی‌اثر |
| `xhttp_mode` / `xhttp_padding` / `xhttp_max_post` / `xhttp_xmux` | تنظیم دقیق XHTTP؛ `xmux` همان چیزی است که ورق‌زدن سایت را سریع می‌کند |
| `reality_server_names` | SNIهای اضافی روی inbound ریلیتی؛ هر کاربر می‌تواند به یکی پین شود |
| `link_test_target` | آدرسی که «تست اتصال» از روی سرور می‌گیرد |

تنظیمات **هر کاربر** (در مودال ویرایش، بخش تنظیمات پیشرفته):

- `flow`: `پیش‌فرض سرور` / `بدون Flow` / `xtls-rprx-vision` / `vision + udp443`
  (خالی = ارث‌بری از پیش‌فرض ترانسپورت؛ «بدون Flow» یک انتخاب واقعی است و لینک هم همان را می‌گوید)
- `policy_level`: پلن سرعت کاربر (`0` متعادل، `1` گیمینگ، `2` استریم/دانلود) — سطوح فقط وقتی ساخته می‌شوند که کسی روی آن‌ها باشد
- `reality_sni` + `short_id`: هویت اختصاصی کاربر روی inbound مشترک (بلاک‌شدن یک SNI بقیه را نمی‌بندد)
- `mux_enabled`: در کانفیگ JSON کلاینت اعمال می‌شود (روی Reality+Vision به‌صورت خودکار حذف می‌شود، چون موتور اجازه نمی‌دهد)

**تست اتصال**: دکمه‌ی «تست اتصال» در جدول کاربران، کانفیگ همان کاربر را از روی سرور بالا
می‌آورد، از طریق SOCKS5 یک صفحه‌ی واقعی می‌گیرد و زمان پاسخ را گزارش می‌کند. اگر کاربر در
کانفیگ در حال اجرا نباشد، همین را به‌عنوان یافته گزارش می‌دهد (به‌جای خطای مبهم).
...
│   ├── links.py         # ساخت لینک‌های اتصال + کانفیگ JSON کلاینت
│   ├── profiles.py      # پروفایل‌های آماده‌ی کانفیگ (recipes)
│   ├── linktest.py      # تست واقعی اتصال (لینت لینک)
